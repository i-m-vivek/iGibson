check_link_pairs = [
    (21, 0),
    (21, 1),
    (21, 2),
    (21, 3),
    (21, 4),
    (21, 5),
    (21, 6),
    (21, 7),
    (21, 8),
    (21, 9),
    (21, 10),
    (21, 11),
    (21, 12),
    (21, 13),
    (21, 14),
    (21, 15),
    (21, 16),
    (21, 17),
    (21, 18),
    (21, 19),
    (21, 47),
    (21, 48),
    (21, 49),
    (22, 0),
    (22, 1),
    (22, 2),
    (22, 3),
    (22, 4),
    (22, 5),
    (22, 6),
    (22, 7),
    (22, 8),
    (22, 9),
    (22, 10),
    (22, 11),
    (22, 12),
    (22, 13),
    (22, 14),
    (22, 15),
    (22, 16),
    (22, 17),
    (22, 18),
    (22, 19),
    (22, 20),
    (22, 47),
    (22, 48),
    (22, 49),
    (23, 0),
    (23, 1),
    (23, 2),
    (23, 3),
    (23, 4),
    (23, 5),
    (23, 6),
    (23, 7),
    (23, 8),
    (23, 9),
    (23, 10),
    (23, 11),
    (23, 12),
    (23, 13),
    (23, 14),
    (23, 15),
    (23, 16),
    (23, 17),
    (23, 18),
    (23, 19),
    (23, 20),
    (23, 47),
    (23, 48),
    (23, 49),
    (24, 0),
    (24, 1),
    (24, 2),
    (24, 3),
    (24, 4),
    (24, 5),
    (24, 6),
    (24, 7),
    (24, 8),
    (24, 9),
    (24, 10),
    (24, 11),
    (24, 12),
    (24, 13),
    (24, 14),
    (24, 15),
    (24, 16),
    (24, 17),
    (24, 18),
    (24, 19),
    (24, 20),
    (24, 47),
    (24, 48),
    (24, 49),
    (25, 0),
    (25, 1),
    (25, 2),
    (25, 3),
    (25, 4),
    (25, 5),
    (25, 6),
    (25, 7),
    (25, 8),
    (25, 9),
    (25, 10),
    (25, 11),
    (25, 12),
    (25, 13),
    (25, 14),
    (25, 15),
    (25, 16),
    (25, 17),
    (25, 18),
    (25, 19),
    (25, 20),
    (25, 47),
    (25, 48),
    (25, 49),
    (26, 0),
    (26, 1),
    (26, 2),
    (26, 3),
    (26, 4),
    (26, 5),
    (26, 6),
    (26, 7),
    (26, 8),
    (26, 9),
    (26, 10),
    (26, 11),
    (26, 12),
    (26, 13),
    (26, 14),
    (26, 15),
    (26, 16),
    (26, 17),
    (26, 18),
    (26, 19),
    (26, 20),
    (26, 47),
    (26, 48),
    (26, 49),
    (27, 0),
    (27, 1),
    (27, 2),
    (27, 3),
    (27, 4),
    (27, 5),
    (27, 6),
    (27, 7),
    (27, 8),
    (27, 9),
    (27, 10),
    (27, 11),
    (27, 12),
    (27, 13),
    (27, 14),
    (27, 15),
    (27, 16),
    (27, 17),
    (27, 18),
    (27, 19),
    (27, 20),
    (27, 47),
    (27, 48),
    (27, 49),
    (28, 0),
    (28, 1),
    (28, 2),
    (28, 3),
    (28, 4),
    (28, 5),
    (28, 6),
    (28, 7),
    (28, 8),
    (28, 9),
    (28, 10),
    (28, 11),
    (28, 12),
    (28, 13),
    (28, 14),
    (28, 15),
    (28, 16),
    (28, 17),
    (28, 18),
    (28, 19),
    (28, 20),
    (28, 47),
    (28, 48),
    (28, 49),
    (29, 0),
    (29, 1),
    (29, 2),
    (29, 3),
    (29, 4),
    (29, 5),
    (29, 6),
    (29, 7),
    (29, 8),
    (29, 9),
    (29, 10),
    (29, 11),
    (29, 12),
    (29, 13),
    (29, 14),
    (29, 15),
    (29, 16),
    (29, 17),
    (29, 18),
    (29, 19),
    (29, 20),
    (29, 47),
    (29, 48),
    (29, 49),
    (30, 0),
    (30, 1),
    (30, 2),
    (30, 3),
    (30, 4),
    (30, 5),
    (30, 6),
    (30, 7),
    (30, 8),
    (30, 9),
    (30, 10),
    (30, 11),
    (30, 12),
    (30, 13),
    (30, 14),
    (30, 15),
    (30, 16),
    (30, 17),
    (30, 18),
    (30, 19),
    (30, 20),
    (30, 47),
    (30, 48),
    (30, 49),
    (31, 0),
    (31, 1),
    (31, 2),
    (31, 3),
    (31, 4),
    (31, 5),
    (31, 6),
    (31, 7),
    (31, 8),
    (31, 9),
    (31, 10),
    (31, 11),
    (31, 12),
    (31, 13),
    (31, 14),
    (31, 15),
    (31, 16),
    (31, 17),
    (31, 18),
    (31, 19),
    (31, 20),
    (31, 47),
    (31, 48),
    (31, 49),
    (32, 0),
    (32, 1),
    (32, 2),
    (32, 3),
    (32, 4),
    (32, 5),
    (32, 6),
    (32, 7),
    (32, 8),
    (32, 9),
    (32, 10),
    (32, 11),
    (32, 12),
    (32, 13),
    (32, 14),
    (32, 15),
    (32, 16),
    (32, 17),
    (32, 18),
    (32, 19),
    (32, 20),
    (32, 47),
    (32, 48),
    (32, 49),
    (33, 0),
    (33, 1),
    (33, 2),
    (33, 3),
    (33, 4),
    (33, 5),
    (33, 6),
    (33, 7),
    (33, 8),
    (33, 9),
    (33, 10),
    (33, 11),
    (33, 12),
    (33, 13),
    (33, 14),
    (33, 15),
    (33, 16),
    (33, 17),
    (33, 18),
    (33, 19),
    (33, 20),
    (33, 47),
    (33, 48),
    (33, 49),
    (34, 0),
    (34, 1),
    (34, 2),
    (34, 3),
    (34, 4),
    (34, 5),
    (34, 6),
    (34, 7),
    (34, 8),
    (34, 9),
    (34, 10),
    (34, 11),
    (34, 12),
    (34, 13),
    (34, 14),
    (34, 15),
    (34, 16),
    (34, 17),
    (34, 18),
    (34, 19),
    (34, 20),
    (34, 47),
    (34, 48),
    (34, 49),
    (35, 0),
    (35, 1),
    (35, 2),
    (35, 3),
    (35, 4),
    (35, 5),
    (35, 6),
    (35, 7),
    (35, 8),
    (35, 9),
    (35, 10),
    (35, 11),
    (35, 12),
    (35, 13),
    (35, 14),
    (35, 15),
    (35, 16),
    (35, 17),
    (35, 18),
    (35, 19),
    (35, 20),
    (35, 47),
    (35, 48),
    (35, 49),
    (36, 0),
    (36, 1),
    (36, 2),
    (36, 3),
    (36, 4),
    (36, 5),
    (36, 6),
    (36, 7),
    (36, 8),
    (36, 9),
    (36, 10),
    (36, 11),
    (36, 12),
    (36, 13),
    (36, 14),
    (36, 15),
    (36, 16),
    (36, 17),
    (36, 18),
    (36, 19),
    (36, 20),
    (36, 47),
    (36, 48),
    (36, 49),
    (37, 0),
    (37, 1),
    (37, 2),
    (37, 3),
    (37, 4),
    (37, 5),
    (37, 6),
    (37, 7),
    (37, 8),
    (37, 9),
    (37, 10),
    (37, 11),
    (37, 12),
    (37, 13),
    (37, 14),
    (37, 15),
    (37, 16),
    (37, 17),
    (37, 18),
    (37, 19),
    (37, 20),
    (37, 47),
    (37, 48),
    (37, 49),
    (38, 0),
    (38, 1),
    (38, 2),
    (38, 3),
    (38, 4),
    (38, 5),
    (38, 6),
    (38, 7),
    (38, 8),
    (38, 9),
    (38, 10),
    (38, 11),
    (38, 12),
    (38, 13),
    (38, 14),
    (38, 15),
    (38, 16),
    (38, 17),
    (38, 18),
    (38, 19),
    (38, 20),
    (38, 47),
    (38, 48),
    (38, 49),
    (39, 0),
    (39, 1),
    (39, 2),
    (39, 3),
    (39, 4),
    (39, 5),
    (39, 6),
    (39, 7),
    (39, 8),
    (39, 9),
    (39, 10),
    (39, 11),
    (39, 12),
    (39, 13),
    (39, 14),
    (39, 15),
    (39, 16),
    (39, 17),
    (39, 18),
    (39, 19),
    (39, 20),
    (39, 47),
    (39, 48),
    (39, 49),
    (40, 0),
    (40, 1),
    (40, 2),
    (40, 3),
    (40, 4),
    (40, 5),
    (40, 6),
    (40, 7),
    (40, 8),
    (40, 9),
    (40, 10),
    (40, 11),
    (40, 12),
    (40, 13),
    (40, 14),
    (40, 15),
    (40, 16),
    (40, 17),
    (40, 18),
    (40, 19),
    (40, 20),
    (40, 47),
    (40, 48),
    (40, 49),
    (41, 0),
    (41, 1),
    (41, 2),
    (41, 3),
    (41, 4),
    (41, 5),
    (41, 6),
    (41, 7),
    (41, 8),
    (41, 9),
    (41, 10),
    (41, 11),
    (41, 12),
    (41, 13),
    (41, 14),
    (41, 15),
    (41, 16),
    (41, 17),
    (41, 18),
    (41, 19),
    (41, 20),
    (41, 47),
    (41, 48),
    (41, 49),
    (42, 0),
    (42, 1),
    (42, 2),
    (42, 3),
    (42, 4),
    (42, 5),
    (42, 6),
    (42, 7),
    (42, 8),
    (42, 9),
    (42, 10),
    (42, 11),
    (42, 12),
    (42, 13),
    (42, 14),
    (42, 15),
    (42, 16),
    (42, 17),
    (42, 18),
    (42, 19),
    (42, 20),
    (42, 47),
    (42, 48),
    (42, 49),
    (43, 0),
    (43, 1),
    (43, 2),
    (43, 3),
    (43, 4),
    (43, 5),
    (43, 6),
    (43, 7),
    (43, 8),
    (43, 9),
    (43, 10),
    (43, 11),
    (43, 12),
    (43, 13),
    (43, 14),
    (43, 15),
    (43, 16),
    (43, 17),
    (43, 18),
    (43, 19),
    (43, 20),
    (43, 47),
    (43, 48),
    (43, 49),
    (44, 0),
    (44, 1),
    (44, 2),
    (44, 3),
    (44, 4),
    (44, 5),
    (44, 6),
    (44, 7),
    (44, 8),
    (44, 9),
    (44, 10),
    (44, 11),
    (44, 12),
    (44, 13),
    (44, 14),
    (44, 15),
    (44, 16),
    (44, 17),
    (44, 18),
    (44, 19),
    (44, 20),
    (44, 47),
    (44, 48),
    (44, 49),
    (45, 0),
    (45, 1),
    (45, 2),
    (45, 3),
    (45, 4),
    (45, 5),
    (45, 6),
    (45, 7),
    (45, 8),
    (45, 9),
    (45, 10),
    (45, 11),
    (45, 12),
    (45, 13),
    (45, 14),
    (45, 15),
    (45, 16),
    (45, 17),
    (45, 18),
    (45, 19),
    (45, 20),
    (45, 47),
    (45, 48),
    (45, 49),
    (46, 0),
    (46, 1),
    (46, 2),
    (46, 3),
    (46, 4),
    (46, 5),
    (46, 6),
    (46, 7),
    (46, 8),
    (46, 9),
    (46, 10),
    (46, 11),
    (46, 12),
    (46, 13),
    (46, 14),
    (46, 15),
    (46, 16),
    (46, 17),
    (46, 18),
    (46, 19),
    (46, 20),
    (46, 47),
    (46, 48),
    (46, 49),
    (21, 33),
    (21, 34),
    (21, 35),
    (21, 36),
    (21, 37),
    (21, 38),
    (21, 39),
    (21, 40),
    (21, 41),
    (21, 42),
    (21, 43),
    (21, 44),
    (21, 45),
    (21, 46),
    (22, 32),
    (22, 33),
    (22, 34),
    (22, 35),
    (22, 36),
    (22, 37),
    (22, 38),
    (22, 39),
    (22, 40),
    (22, 41),
    (22, 42),
    (22, 43),
    (22, 44),
    (22, 45),
    (22, 46),
    (23, 32),
    (23, 33),
    (23, 34),
    (23, 35),
    (23, 36),
    (23, 37),
    (23, 38),
    (23, 39),
    (23, 40),
    (23, 41),
    (23, 42),
    (23, 43),
    (23, 44),
    (23, 45),
    (23, 46),
    (24, 32),
    (24, 33),
    (24, 34),
    (24, 35),
    (24, 36),
    (24, 37),
    (24, 38),
    (24, 39),
    (24, 40),
    (24, 41),
    (24, 42),
    (24, 43),
    (24, 44),
    (24, 45),
    (24, 46),
    (25, 32),
    (25, 33),
    (25, 34),
    (25, 35),
    (25, 36),
    (25, 37),
    (25, 38),
    (25, 39),
    (25, 40),
    (25, 41),
    (25, 42),
    (25, 43),
    (25, 44),
    (25, 45),
    (25, 46),
    (26, 32),
    (26, 33),
    (26, 34),
    (26, 35),
    (26, 36),
    (26, 37),
    (26, 38),
    (26, 39),
    (26, 40),
    (26, 41),
    (26, 42),
    (26, 43),
    (26, 44),
    (26, 45),
    (26, 46),
    (27, 32),
    (27, 33),
    (27, 34),
    (27, 35),
    (27, 36),
    (27, 37),
    (27, 38),
    (27, 39),
    (27, 40),
    (27, 41),
    (27, 42),
    (27, 43),
    (27, 44),
    (27, 45),
    (27, 46),
    (28, 32),
    (28, 33),
    (28, 34),
    (28, 35),
    (28, 36),
    (28, 37),
    (28, 38),
    (28, 39),
    (28, 40),
    (28, 41),
    (28, 42),
    (28, 43),
    (28, 44),
    (28, 45),
    (28, 46),
    (29, 32),
    (29, 33),
    (29, 34),
    (29, 35),
    (29, 36),
    (29, 37),
    (29, 38),
    (29, 39),
    (29, 40),
    (29, 41),
    (29, 42),
    (29, 43),
    (29, 44),
    (29, 45),
    (29, 46),
    (30, 32),
    (30, 33),
    (30, 34),
    (30, 35),
    (30, 36),
    (30, 37),
    (30, 38),
    (30, 39),
    (30, 40),
    (30, 41),
    (30, 42),
    (30, 43),
    (30, 44),
    (30, 45),
    (30, 46),
    (31, 32),
    (31, 33),
    (31, 34),
    (31, 35),
    (31, 36),
    (31, 37),
    (31, 38),
    (31, 39),
    (31, 40),
    (31, 41),
    (31, 42),
    (31, 43),
    (31, 44),
    (31, 45),
    (31, 46),
    (32, 34),
    (32, 35),
    (32, 36),
    (32, 37),
    (32, 38),
    (32, 39),
    (32, 40),
    (32, 41),
    (32, 42),
    (32, 43),
    (32, 44),
    (32, 45),
    (32, 46),
    (33, 35),
    (33, 36),
    (33, 37),
    (33, 38),
    (33, 39),
    (33, 40),
    (33, 41),
    (33, 42),
    (33, 43),
    (33, 44),
    (33, 45),
    (33, 46),
    (34, 36),
    (34, 37),
    (34, 38),
    (34, 39),
    (34, 40),
    (34, 41),
    (34, 42),
    (34, 43),
    (34, 44),
    (34, 45),
    (34, 46),
    (35, 37),
    (35, 38),
    (35, 39),
    (35, 40),
    (35, 41),
    (35, 42),
    (35, 43),
    (35, 44),
    (35, 45),
    (35, 46),
    (36, 38),
    (36, 39),
    (36, 40),
    (36, 41),
    (36, 42),
    (36, 43),
    (36, 44),
    (36, 45),
    (36, 46),
    (37, 39),
    (37, 40),
    (37, 41),
    (37, 42),
    (37, 43),
    (37, 44),
    (37, 45),
    (37, 46),
]

part_index = [
    "base_footprint",
    "base_link",
    "base_laser_link",
    "base_sonar_03_link",
    "base_sonar_02_link",
    "base_sonar_01_link",
    "base_antenna_left_link",
    "base_antenna_right_link",
    "base_imu_link",
    "ignore_suspension_right_link",
    "wheel_right_link",
    "ignore_suspension_left_link",
    "wheel_left_link",
    "ignore_caster_front_right_1_link",
    "ignore_caster_front_right_2_link",
    "ignore_caster_front_left_1_link",
    "ignore_caster_front_left_2_link",
    "ignore_caster_back_right_1_link",
    "ignore_caster_back_right_2_link",
    "ignore_caster_back_left_1_link",
    "ignore_caster_back_left_2_link",
    "torso_fixed_link",
    "torso_lift_link",
    "head_1_link",
    "head_2_link",
    "xtion_link",
    "xtion_optical_frame",
    "xtion_orbbec_aux_joint_frame",
    "xtion_rgb_frame",
    "xtion_depth_frame",
    "xtion_depth_optical_frame",
    "eyes",
    "xtion_rgb_optical_frame",
    "arm_1_link",
    "arm_2_link",
    "arm_3_link",
    "arm_4_link",
    "arm_5_link",
    "arm_6_link",
    "arm_7_link",
    "arm_tool_link",
    "wrist_ft_link",
    "wrist_ft_tool_link",
    "gripper_tool_link",
    "gripper_link",
    "gripper_right_finger_link",
    "gripper_left_finger_link",
    "gripper_grasping_frame",
    "torso_fixed_column_link",
    "base_cover_link",
    "rgbd_laser_link",
]
fetch_links = [
    "base_link",
    "caster_wheel_link",
    "r_wheel_link",
    "l_wheel_link",
    "torso_lift_link",
    "head_pan_link",
    "head_tilt_link",
    "head_camera_link",
    "head_camera_rgb_frame",
    "head_camera_rgb_optical_frame",
    "head_camera_depth_frame",
    "head_camera_depth_optical_frame",
    "eyes",
    "shoulder_pan_link",
    "shoulder_lift_link",
    "upperarm_roll_link",
    "elbow_flex_link",
    "forearm_roll_link",
    "wrist_flex_link",
    "wrist_roll_link",
    "gripper_link",
    "estop_link",
    "laser_link",
    "torso_fixed_link",
]
# for i, j in check_link_pairs:
#     print(part_index[i+1], part_index[j+1])


from gibson2.external.pybullet_tools.utils import pairwise_link_collision
from gibson2.scenes.stadium_scene import StadiumScene
from gibson2.simulator import Simulator

# from gibson2.robots.tiago_single_robot import Tiago_Single
from gibson2.robots.fetch_robot import Fetch
from gibson2.utils.utils import parse_config
from gibson2.render.mesh_renderer.mesh_renderer_settings import MeshRendererSettings

config = parse_config("configs/tiago_stadium_config.yaml")

settings = MeshRendererSettings(
    enable_shadow=False, msaa=False, enable_pbr=True, texture_scale=1.0
)
s = Simulator(
    mode="headless",
    image_width=128,
    image_height=128,
    vertical_fov=90,
    rendering_settings=settings,
    physics_timestep=1 / 240.0,
    render_timestep=1 / 10.0,
    render_to_tensor=False,
)


scene = StadiumScene()
s.import_scene(scene, load_texture=False)
my_robot = Fetch(config)
my_robot.self_collision = True
s.import_robot(my_robot)
my_robot.set_position([0, 0, 0])


body = my_robot.robot_ids[0]
print(part_index[22], part_index[48])
print("NNN")
for link1, link2 in check_link_pairs:
    if part_index[link1+1] == "torso_fixed_link" or part_index[link2+1] == "torso_fixed_link":
        print(part_index[link1+1], part_index[link2+1])  
    ans = pairwise_link_collision(body, link1, body, link2)
    if ans:
        print(
            f"(link_from_name(self.robot_id, '{part_index[link1+1]}'), link_from_name(self.robot_id, '{part_index[link2+1]}')),"
        )
        # print(part_index[link1+1], part_index[link2+1])
# 21 47

# We can improve the collision by checking in which links collision happens while standing still
# pairwise_collision
