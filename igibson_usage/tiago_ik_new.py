...
def main():
    ...
    s = Simulator(mode='headless', physics_timestep=1 / 240.0)
    scene = EmptyScene()
    ...
    joints = [j.joint_index for j in robot.ordered_joints[2:-2]] 
    ...
    max_limits = get_max_limits(robot_id, joints)
    min_limits = get_min_limits(robot_id, joints)
    rest_position = list(get_joint_positions(robot_id, joints))
    ...
    def accurateCalculateInverseKinematics(robotid, endEffectorId, targetPos, threshold, maxIter):
        ...
        while it < maxIter:
            ...
            print(len(joints), len(jointPoses))
            set_joint_positions(robotid, joints, jointPoses)
