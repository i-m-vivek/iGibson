...
# SAC With a Hybrid action space (SEPARATE approximator for the discrete part)
...
class SAC_hybridPolicy(Policy):
    ...
    def __init__(self, mu_approximator, sigma_approximator, discrete_approximator,
        ...
        max_a = max_a[:mu_approximator.output_shape[0]]
        min_a = min_a[:mu_approximator.output_shape[0]]
    ...
    def entropy(self, state=None):
        ...
        """
        return torch.mean(self.cont_distribution(state).entropy()).detach().cpu().numpy().item() + torch.mean(self.discrete_distribution(state).entropy()).detach().cpu().numpy().item()
...
class SAC_hybrid(DeepAC):
    ...
    * With a Hybrid action space (SEPARATE approximator for the discrete part)
